...
def benchmark(debug: bool = True) -> None:
    ...
    if not samples_dir.exists() or not samples_dir.is_dir():
        console.print(f"[bold red]Sample-Verzeichnis nicht gefunden:[/bold red] {samples_dir}")
        debugger.save_text(f"Sample-Verzeichnis nicht gefunden: {samples_dir}\n", "Samples suchen")
        return

    sample_files = sorted(samples_dir.glob("*.wav"))
    if sample_files:
        console.print(f"[bold green]Gefundene Sample-Dateien:[/bold green] {[f.name for f in sample_files]}")
        debugger.save_text(f"Sample-Dateien: {[f.name for f in sample_files]}\n", "Samples suchen")
    if not sample_files:
        console.print(f"[bold yellow]Keine .wav-Dateien in {samples_dir} gefunden[/bold yellow]")
        ...
        try:
            for sample in sample_files:
                ...
                if not transcript or not transcript.strip():
                    status.update("[bold red]Es konnte kein Audio transkribiert werden.[/bold red]")
                    debugger.save_text("Es konnte kein Audio transkribiert werden.\n", "Transkribieren")
                    time.sleep(1)
                    continue

                debugger.save_text(transcript, "Transkript | in " + stt_time.__format__(".2f") + " Sekunden")
                ...
                if not cleaned:
                    status.update("[bold yellow]Nur Wakeword erkannt, keine eigentliche Eingabe.[/bold yellow]")
                    debugger.save_text("Nur Wakeword erkannt, keine eigentliche Eingabe.\n", "Transkript bereinigen")
                    time.sleep(1)
                    continue
                ...
                if not answer or not answer.strip():
                    status.update("[bold red]Es konnte keine Antwort generiert werden.[/bold red]")
                    debugger.save_text("Es konnte keine Antwort generiert werden.\n", "Antwort generieren")
                    time.sleep(1)
                    continue

                debugger.save_text(answer, "Antwort | in " + llm_time.__format__(".2f") + " Sekunden")
                status.update("[bold green]Antwort:[/bold green] " + answer)
                try:
                    tts_start = time.perf_counter()
                    tts.stream_speak(answer)
                    tts_time = time.perf_counter() - tts_start
                    debugger.save_text(f"\n", "Sprechen | in " + tts_time.__format__(".2f") + " Sekunden")
